...
AUCell and RcisTarget were updated recently (May 2018). Please, make sure you have the correct versions:
**AUCell 1.2.1**, and **RcisTarget 1.0.1** or posterior.
...
In case you have trobule installing the packages with `biocLite` (for example, if you don't have the newest version of R), you may try installing them directly from the Bioconductor links:
```{r alternativeDownload, eval=FALSE}
install.packages("https://bioconductor.org/packages/release/bioc/src/contrib/AUCell_1.2.2.tar.gz", repos=NULL)
install.packages("https://bioconductor.org/packages/release/bioc/src/contrib/RcisTarget_1.0.2.tar.gz", repos=NULL)
install.packages("https://bioconductor.org/packages/release/bioc/src/contrib/GENIE3_1.2.0.tar.gz", repos=NULL)
```
...
In addition to the R-packages, you will also need to download the species-specific databases for RcisTarget (the motif rankings). The links to all the available databases are available in [our website](https://resources.aertslab.org/cistarget/).
...
Download the *.feather* and *.descr* files for the relevant organism *(feel free to use any alternative download method)* [Aprox file size: 1GB]:
...
vignetteFile <- paste(pkgDir, "SCENIC_Running.Rmd", sep="/")
...
```{r GEOdownload, cache=TRUE, results='hide', message=FALSE, eval=FALSE}
