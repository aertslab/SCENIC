...
To edit the thresholds, we propose to go through the plots on the pdf (*3.4_histograms_AUCell_tSNE.pdf*), and change the thresholds value in the following text file (edit field *threshold*). 
...
Create file to edit:
```{r thresholds2edit}
load("int/3.4_cells_AUCellThresholds.RData")
...
```{r manualThresholds}
manualThresholds <- read.table("int/3.5_2_AUCellThresholds_toEDIT.txt", sep="\t", header=T)
...
```{r bimodalThresholds, eval=TRUE, fig.height=7, fig.width=10}
