...
**AUCell 1.2.4**, **RcisTarget 1.0.2**, and **GENIE3 1.2.1** or posterior.
...
*To avoid problems or incomplete downloads (especially if you have a slow connection), we recommend to use [zsync_curl](https://resources.aertslab.org/cistarget/help.html). If you prefer to download directly from R, you can use the following code:*
...
This tutorial is run on a real dataset to illustrate the results obtained at each step. However, keep in mind that some steps of the workflow (i.e. GENIE3, motif enrichment) can take hours to run. Feel free to use your own dataset instead, or load the intermediate results from the example (some are available in [Github](https://github.com/aertslab/SCENIC/)).
