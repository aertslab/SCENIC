...
To apply SCENIC to other species, it would require manual adjustments on the second step (e.g. new RcisTarget databases or using a diferent motif-enrichment-analysis tool).
...
The choice of input expression matrix might have some effect in the creation of the regulons (due to the co-expression analysis, the expression is not taken into account for the motif analysis), but not on the scoring of the regulons on the cells (AUCell is cell ranking-based, which works as an implicit normalization). Overall, SCENIC is quite robust to this choice, we have applied SCENIC to datasets using raw (logged) UMI counts, normalized UMI counts, and TPM and they all provided reliable results (see *@aibar2017*).
...
AUCell and RcisTarget were updated in January 2019. Please, make sure you have the recommended versions:
...
In case you have trobule installing the packages from Bioconductor (for example, for older versions of R), you may try installing them from Github *or* directly from the Bioconductor package files:
...
sce <- load_as_sce(loomPath) # any SingleCellExperiment object
