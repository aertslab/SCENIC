...
abstract: "Second step of SCENIC workflow: How to identify regulons (direct TF targets) based on DNA motif enrichment with RcisTarget"
output: 
...
> *Expected running time:* This step will likely take a few hours to run (you might want to run it on a separate console...). The running time of this step depends on the number and size of the rankings to evaluate (in this case, 2 rankings with `r ncol(motifRankings[[1]])` motifs), and the number of gene-sets (`r length(tfModules)`) (i.e. not on the number of cells).
