...
For tests (quick runs) this subset with only 200 cells and ~1800 genes is included in the package:
`exprMatPath <- system.file(package="SCENIC", "examples/sceMouseBrain_mini.Rds")`
...
```{r checkMissingGenes}
interestingGenes <- c("Neurod1", "Sox10", "Dlx5")
# any missing?
...
## Option B: GRNboost (Python)  - TODO: remove
...
We are now ready to run GRNBoost. You can find the tutorial with some examples in  https://arboreto.readthedocs.io/en/latest/examples.html. Once it has finished running, you can import the output from GRNBoost back to R following the tutorial `vignette("detailedStep_1_coexNetwork2modules")` (starting from a "linkList", you might need to adjust the weight thresholds).
...
# Optional steps:
## Creating/comparing t-SNEs

The cells can now be grouped based on the AUC (*See the section below "Exploring > Cell states" for details*). 
Since we use t-SNE as visualization, it is recommended to try different clustering/t-SNE settings to evaluate the stability of the states. Feel free to use UMAP (or trajectory inference methods, if appropriate) instead.
