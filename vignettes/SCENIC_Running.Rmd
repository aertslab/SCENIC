...
scenicOptions <- initializeScenic(org=org, dbDir=dbDir, datasetTitle="SCENIC example on Mouse brain", nCores=20) 
...
We will soon create detailed tutorials/notebooks explaining the internal code of each of these functions.
<!-- We will soon create detailed tutorials/notebooks explaining the internal code of each of these functions (`vignette(package="SCENIC")`). These might be useful for users who want to know the details of the implementation, or to modify or run only some of the steps of the workflow. -->
...
The `.loom` file can be created with the function `export2scope()` (requires the package [SCopeLoomR](https://github.com/aertslab/SCopeLoomR/)). This function automatically adds the SCENIC data into the .loom file. 
...
# Export:
scenicOptions@fileNames$output["loomFile",] <- "mouseBrain"
