---
title: "Running SCENIC: Step 1 and 'runSCENIC' (automated steps 2-4)"
subtitle: SCENIC (Single Cell rEgulatory Network Inference and Clustering)
abstract: "First steps for running SCENIC: Filtering the expression matrix, running GENIE3, and (optional) running the next steps in SCENIC using runSCENIC(), the wrapper function."
output:
...
- `seed`: To reproduce the results in a future run. Since GENIE3 is based on a Random Forest approach, each time it is run the results will be slightly different. The higher the number of trees used (`ntrees`), the lower the variability it will be. The seed allows to reproduce exact results in multiple runs.
...
# Run GENIE3
weightMatrix <- GENIE3(exprMatrix_filtered, regulators=inputTFs, 
                       ncores=4, seed=123)
